    ...
    while True:
        ...
        try:
            for db, start, length in zip(db_numbers, start_addresses, data_lengths):
                ...
                rotate = random.randint(4000,10000)
                data_list.append(rotate)
                data_list.extend(round(random.uniform(-1, 1), 3) for i in range(20000))
                print(data_list)
...
if __name__ == '__main__':
    ...
    ip_addresses = [f'192.99.7.1']  # 示例 IP 列表
    ...
    #     else:
    #         stable_sql1 += f"`{field_name1}` float )"
    stable_sql1 += ','.join(f"`{varname.name}` float" for varname in DataName)
