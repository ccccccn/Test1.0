...
def data_analysis():
    ...
    for sub_table in table_name:
        ...
        # 获取各变量个数
        tcur.execute(
            f"select count(*) from `{sub_table}` WHERE ts BETWEEN '{day_before}' AND '{day_now}' and `飞轮转速` > 4800")
            ...
            for i, tag in zip(range(4, len(tmp_column_name) - 3, 5), tags):
                ...
                if tag == '积分电流':
                    ...
                    query = (query + " , ".join(condition) +
                             ...
                             f"where ts BETWEEN '{day_before}' and '{day_now}' and `飞轮转速` > 4800 and (" + " or ".join(
                                conditions) + ")")
                    ...
                    query = (query + " , ".join(condition) +
                             ...
                             f"MIN(`飞轮舱温度`) as `tempetature_min`,MIN(`飞轮转速`) as `rotate_min`"
                             f" from threadtestt4.`{sub_table}` "
                             f"where ts BETWEEN '{day_before}' and '{day_now}' and `飞轮转速` > 4800")
                    all_query.append(query)
                print("当前查询语句为:", query)
                tcur.execute(query)
                data = tcur.fetchall()
