...
async def get_plc_data(selected_ip, plc_ip, plc_rack, plc_slot, db_numbers, start_addresses, data_lengths):
    try:
        plc = Client()
...
def process_ip(plc_info, selected_ip):
    ...
    asyncio.run(get_plc_data(ip, rack, slot, db_numbers, start_addresses, data_lengths, selected_ip))
